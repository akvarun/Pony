...
actor Boss
  ...
  new create(env: Env, n: USize, k: USize) =>
    ...
    if n <= 500 then
      ...
      _chunk_size = MathUtils.ceil(MathUtils.sqrt(n))
    end
    ...
    for i in Range(0, num_workers) do
      _workers.push(Worker(env, k, i * _chunk_size))
    end
    ...
    for i in Range(0, num_workers) do
      ...
      try
        _workers(i)?.calculate_sum_of_squares(start, endp)
      end
    end
...
actor Worker
  ...
  var _sum: U64
  var _index: USize
  let _checker: PerfectSquareValidator
  ...
  new create(env: Env, k: USize, offset: USize) =>
    ...
    _sum = 0
    _index = 0
    _checker = PerfectSquareValidator(env)

  be calculate_sum_of_squares(start: USize, endp: USize) =>
  ...
  fun ref add_square(square: U64, start: USize) =>
    ...
    if _queue.size() == _k then
      _checker.validate_perfect_square(_sum, _offset + _index)
      _index = _index + 1
    end


actor PerfectSquareValidator
  let _env: Env
  ...
  be validate_perfect_square(n: U64, index: USize) =>
